...
export default function Log(
  ...
  eventBus.on(SCOPE_DESTROYED_EVENT, event => {
    ...
    const text = `${
      isSubProcess ? (getElementName(scopeElement) || 'SubProcess') : '流程'
    }${
      completed ? '结束' : '取消'
    }`;
  ...
  eventBus.on(TRACE_EVENT, event => {
    ...
    if (is(element, 'bpmn:BusinessRuleTask')) {
      ...
      if (isTypedEvent(getBusinessObject(element), 'bpmn:TerminateEventDefinition')) {
        this.log({
          text: elementName || '终止结束事件',
          icon: 'bpmn-icon-end-event-terminate',
          scope
        });
      } else {
        this.log({
          text: elementName || '结束事件',
...
Log.prototype._init = function() {
  this._container = domify(`
    <div class="bts-log hidden">
      ...
      <div class="bts-content">
        <p class="bts-entry placeholder">没有条目</p>
      </div>
    </div>
  `);
  ...
  this.paletteEntry = domify(`
    <div class="bts-entry" title="显示模拟日志">
      ${ LogIcon() }
    </div>
  `);
...
Log.prototype.clear = function() {
  ...
  this._placeholder = domify('<p class="bts-entry placeholder">没有条目</p>');
